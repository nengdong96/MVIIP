...
from network import Model, Res50IBNaBNNeck, Classifier, Classifier2
from tools import CrossEntropyLabelSmooth, ReasoningLoss, KLDivLoss, os_walk
...
class Base:
    ...
    def _init_optimizer(self):
        ...
        classifier2_params_group = [{'params': self.classifier2.parameters(),
                                     'lr': self.learning_rate, 'weight_decay': self.weight_decay}]
        ...
        self.classifier2_lr_scheduler = WarmupMultiStepLR(self.classifier2_optimizer, self.milestones,
                                                          gamma=0.1, warmup_factor=0.01, warmup_iters=10)
    ...
    def save_model(self, save_epoch, is_best):
        if is_best:
            ...
            classifier2_file_path = os.path.join(self.save_model_path, 'classifier2_{}.pth'.
                                                 format(save_epoch))
        ...
        if self.max_save_model_num > 0:
            ...
            for file in files:
                if '.pth' not in file:
                    files.remove(file)
            if len(files) > 3 * self.max_save_model_num:
                ...
                classifier2_file_path = os.path.join(root, 'classifier2_{}.pth'.
                                                     format(file_iters[0]))
    ...
    def resume_model(self, resume_epoch):
        ...
        classifier2_path = os.path.join(self.save_model_path, 'classifier2_{}.pth'.
                                        format(resume_epoch))
